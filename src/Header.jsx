// Fájl: src/Header.jsx (VÉGLEGES, FELTURBÓZOTT VERZIÓ)
...
function Header() {
  ...
  const [kinalatData, setKinalatData] = useState(null);
  ...
  const fetchKinalat = () => {
    setLoading(true);
    setError(null);
    fetch(`https://nyaloda-backend.onrender.com/api/stock/all-counters`)
      .then(res => {
        if (!res.ok) throw new Error('A szerver nem válaszol. Próbáld újra később.');
        return res.json();
      })
      .then(data => {
        setKinalatData(data.storesWithStock);
        setLoading(false);
      })
      .catch(err => {
        setError(err);
          ...
          <div className="dropdown-container">
            ...
            {isPultOpen && (
              <div className="dropdown-menu live-kinalat">
                ...
                {error && <div className="kinalat-status error">{error.message}</div>}
                    ...
                    {!selectedStoreId && (
                      <>
                        <div className="kinalat-title">
                          ...
                          <button className="kinalat-refresh-button" onClick={fetchKinalat} title="Frissítés">
                            <RefreshIcon />
                          </button>
                        </div>
